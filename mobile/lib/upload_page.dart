...
class _Uploader extends State<Uploader> {
  ...
  initState() {
    ...
    // TODO: update the location from where the foto is taken
  ...
  getVideoThumbnail(FlutterVideoCompress flutterVideoCompress, videoFile) async {
    final thumbnailFile =
        await flutterVideoCompress.getThumbnailWithFile(videoFile.path,
  ...
  getCompressedVideo(flutterVideoCompress, videoFile)  async {
    print('starting compression of video');
    await flutterVideoCompress.compressVideo(
      videoFile.path,
      quality: 
    ...
    return file == null
        ...
        : Scaffold(
            ...
            body: ListView(
              children: <Widget>[
                PostForm(
                  imageFile: isVideo ? thumbnail : file,
          ...
          children: <Widget>[
            ...
            SimpleDialogOption(
                ...
                onPressed: () async {
                  ...
                  // await getCompressedVideo(flutterVideoCompress, imageFile);
                  final videoThumbnail = await getVideoThumbnail(flutterVideoCompress, imageFile);
                  setState(() {
                    file = imageFile;
...
//    Im.Image newim = Im.remapColors(image, alpha: Im.LUMINANCE);
  ...
  void postImage() {
    ...
    uploadImage(file, isVideo)
    .then((String data) async {
      fileUrl = data;
      if(isVideo) {
        uploadImage(thumbnail, isVideo);
      } else {
...
Future<String> uploadImage(var imageFile, bool isVideo) async {
  var uuid = Uuid().v1();
  var name = isVideo ? "post_$uuid.mp4" : "post_$uuid.jpg";
...
void postToFireStore(
    {String mediaUrl, String thumbnailUrl, String location, String description}) async {
